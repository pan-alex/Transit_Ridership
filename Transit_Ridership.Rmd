...
#### Exploratory Plots (monthly ridership)

```{r}
ggplot(chicago_by_month) +
    geom_point(aes(x = DateTime, y = avg_weekday_rides)) +
...
```{r}
ggplot(chicago_by_month) +
    aes(x = DateTime, y = avg_weekday_rides, col = month.abb[Month]) +
    geom_point(alpha = 0.6) +
    geom_line(aes(group = month.abb[Month]), alpha = 0.6) +
    ylab('Mean Weekday Ridership')
```

Nothing super interesting going on here.
...
ggplot(temp) +
    ...
    geom_boxplot(alpha = 0.1, size = 0.75) +
...
```{r}
# Ten busiest stations
head(chicago_by_station %>% arrange(desc(avg_saturday_rides)), 10)
```

(I don't care enough about busiest Saturdays to plot them)
